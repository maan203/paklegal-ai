...
**Prepare before you go to the police station or a lawyer.** Describe what happened in Urdu or English, by voice or text, or upload a legal document, and get a plain-language explanation grounded in the official text of Pakistani law, with the exact Articles and Sections it used.
...
Two everyday situations where people are at a disadvantage:

1. **Reporting an incident out loud.** At a police station (or a lawyer's office) people explain what happened verbally, under stress, while someone else writes it down. Details get missed or told out of order, and they don't know which laws apply or what their rights are.
...
|---|---|---|
| 1 | **Describe Your Incident** (main feature) | The person speaks (transcribed with Whisper) or types the incident in Urdu, Roman Urdu or English. The system extracts a structured incident record, which the person **reviews and corrects**; retrieves the provisions that may apply and explains the law, their rights and next steps with sources; and prepares a **draft complaint for FIR registration** and a **case summary for their lawyer** from the reviewed facts. Follow-up questions are answered in the same context. It is a preparation tool: it files nothing, and the police record the FIR. |
...
```mermaid
flowchart LR
  A[Incident or document<br/>voice via Whisper / text / PDF / photo] --> B[1. Understand<br/>structured fact extraction<br/>JSON schema]
  ...
  E --> F[Answer + sources]
  B --> R[Person reviews and<br/>corrects the facts]
  R --> G[Draft complaint +<br/>lawyer case summary<br/>built by code]
...
The **draft complaint for FIR registration** and the **case summary** are assembled by plain code from the facts the person reviewed, not generated by the model, so they contain only what the person confirmed. The complaint names no PPC sections: deciding the offence is for the police, and a wrong section in a complainant's own statement can hurt their case. Sections appear only in the explanation, as "may apply", with sources.
...
- **"Draft complaint", never "FIR".** The police record the FIR under s.154 CrPC; a citizen submits a written complaint. The app never claims to produce an FIR or anything with legal effect.
- **A human review step before any document.** The AI extracts the facts, the person corrects them, and only then is the draft built. Speech transcripts are also shown for correction first, since names, places and amounts are where transcription fails (Whisper wrote "Sadar" for "Saddar" in testing).
- **Whisper instead of browser speech recognition.** Works in every browser (browser speech recognition is Chrome/Edge only) and supports Urdu; the language is set explicitly so Urdu speech is written in Urdu script rather than Hindi.
...
- **Documents and speech:** `unpdf` for PDF text; Groq Whisper (`whisper-large-v3`) for voice, recorded with `MediaRecorder`
...
│   ├── incident.tsx        # Describe Your Incident
...
│   ├── FactsEditor.tsx     # Review and correct the extracted facts
│   ├── EditableDraft.tsx   # Editable complaint draft (copy / print)
│   └── DocumentResult.tsx  # Copy / print for generated documents
├── lib/
│   ├── ai-functions.ts     # Server functions: extraction, grounded answers, search
│   ├── case-summary.ts     # Draft complaint + lawyer case summary (built by code)
│   └── rag/
...
└── hooks/                  # useLegalChat, useVoiceRecorder, useLocalStorage
